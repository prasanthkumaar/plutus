...
- **Child ticket**: an issue linked to the map as a GitHub sub-issue using `gh api` on the sub-issues endpoint. Where sub-issues are unavailable, add the child to a task list in the map body and put `Part of #<map>` at the top of the child body. Use `wayfinder:<type>` labels for `research`, `prototype`, `grilling`, or `task`. Once claimed, assign the ticket to the driving developer.
...
- **Resolve**: comment with the answer, close the issue, then append a context pointer with its gist and link to the map's Decisions-so-far.
